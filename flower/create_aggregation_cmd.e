class CREATE_AGGREGATION_CMD inherit
    -- Add an aggregation  class to the diagram identified by the
...
feature		-- Attributes
	...
	name : STRING is "create_aggregation";
...
feature { TCL_INTERPRETER }		-- Command invocation
	...
	evaluate (args : ARRAY[STRING]) : STRING is
		local
			id : INTEGER;
			diagram : DIAGRAM;
			command : DRAW_AGGREGATION_CMD;
		do
			if args.lower /= args.upper 
				or else (not (args @ 0).is_integer)
			then
				raise("Error in arguments:  should be 'eiffel create_aggregation <id>'");
			end
...
end -- class CREATE_AGGREGATION_CMD
